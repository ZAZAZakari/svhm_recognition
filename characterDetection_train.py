...
with graph.as_default():
    ...
    tfTrainingLabels = tf.placeholder(tf.float32, [None, 2], name='tfTrainingLabels')
...
with tf.Session(graph=graph) as session:
    ...
    while (True):
        ...
        _, stepLoss, stepAcc = session.run([optimizer, loss, accuracy], \
                                    feed_dict={tfTrainingData: batchData, tfTrainingLabels: batchLabel, keep_prob: 0.5})
        step += 1
        if (step == 1 or step % 100 == 0):
            ...
            print ("Step %5d: Loss = %10.6f, TrainAcc = %10.6f, validLoss = %10.6f, validAcc = %10.6f" % (step, stepLoss, stepAcc, validLoss, validAcc))

        if (step > 100):
            save_path = saver.save(session, "model/temp.ckpt", global_step=0)
            tf.train.write_graph(session.graph_def, 'model', 'temp.pb', False)
            freeze_graph.freeze_graph('model/temp.pb', '', True, 'temp.ckpt-0', 'Output/softmax,Output/loss', 'save/restore_all',
                                      'save/Const:0', "model/temp_out.pb", False, "") 
            break
# ==========================================================================================================================
# End of code
# ==========================================================================================================================